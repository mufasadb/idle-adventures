...
// weaponBuff (D59): an OPTIONAL active enhancement — absent = today's behaviour
...
export type ExchangeResult = {
  ...
  victory: boolean;
  defeated: boolean;
  weaponBuffAfter?: { id: string; charges: number }; // D59: charges after this strike (undefined = cleared/none)
  poisonAfter?: { dmg: number; rounds: number }; // D59: engagement poison after this round's tick (undefined = none)
  poisonDmg: number; // D59: poison damage dealt to the monster this round (0 = none)
};

// One combat round (si7.1): player strike → weapon-enhancement bookkeeping (D59:
...
// weaponBuff/poison (D59): absent = today's behaviour exactly (all combat numbers
...
export function strikeExchange(
  ...
  // Weapon-enhancement bookkeeping (D59). The strike always spends one charge; at
...
// Atomic combat (sim/harness API). Reads Expedition.weaponBuff at fight start (D59)
