...
export type Loadout = {
  ...
  enhancements?: ItemStack[]; // weapon enhancements (D59): whetstone/oils packed like battle-items (1 slot/unit, no stacking); applied mid-run by the `enhance` action, unused ones bank back. Optional/absent = []; reads guard with `?? []`.
};
...
export type LoadoutSlot =
  ...
  | "enhancement"; // weapon enhancements (D59): whetstone/oils, packed like a battle-item (1 slot/unit, no stacking)
...
export type Action =
  ...
  | { type: "enhance"; id: string } // apply a weapon enhancement (D59): sets Expedition.weaponBuff from a held enhancement stack; usable engaged or unengaged, no exchange, no energy (mirrors use-item/quaff)
...
export type GameEvent =
  ...
  | { type: "exchanged"; creature: string; dmgDealt: number; dmgTaken: number; monsterHp: number; hp: number; potionsUsed: number; arrowSpent?: boolean; poisonDmg?: number } // arrowSpent (D45): present when this exchange shot an arrow. poisonDmg (D59): poison DoT dealt to the monster this round, present when >0
  ...
  | { type: "enhanced"; id: string; charges: number } // weapon enhancement applied (D59): the coating `id` now rides the expedition with `charges` strikes left
