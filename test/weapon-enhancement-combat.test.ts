...
test("playerDamage: flatDamage raises the swing; absent buff is byte-identical (D60)", () => {
...
test("playerDamage: affinityTag coating fires ×AFFINITY_MULTIPLIER vs a matching monster tag (D60)", () => {
  const l = emptyLoadout();
...
test("playerDamage: coating affinity does NOT double when the weapon already matches — matched-or-not (D60)", () => {
  const l = emptyLoadout();
...
for (const [label, buffId, weapon, creature] of [
  ...
  test(`atomic resolveCombat == interactive fight for a coated fight — ${label} (D60)`, () => {
...
test("charges decrement per strike and clear at 0 (D60)", () => {
...
test("poison ticks each round on the engagement and lands a kill the swing alone wouldn't (D60)", () => {
