...
test("constants: ENHANCEMENT ⇔ WEAPON_ENHANCEMENT are the same set, and slotOf → enhancement (D59)", () => {
...
test("enhance: applies a coating, sets weaponBuff at full charges, decrements the stack, emits enhanced (D59)", () => {
...
test("enhance: applying over an existing buff REPLACES it — old charges discarded (D59)", () => {
  const before = expeditionWith();
...
test("enhance: works while ENGAGED, runs no exchange, spends no energy (D59)", () => {
...
test("enhance: rejects wrong-slot / insufficient / not-on-expedition (D59)", () => {
  const before = expeditionWith();
