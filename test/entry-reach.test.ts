...
//     capacity of the entry, so the opening move isn't forced. Min observed was 4
//     (rare tundra maps — ice costs 2× so one capacity reaches fewer nodes, which the
//     M7 memo calls emergent-and-good, NOT a boxed entry: those same maps still
//     flood 690+ tiles). WITHIN_CAPACITY_FLOOR=3 leaves one unit of RNG slack under
...
// close note): a hard K=5 within-capacity fails on ~2/408 tundra maps whose entries
...
// invariant is expressed on FLOOD (tight) and the first-run choice on within-capacity
...
test("si7.5 structural: entry opens onto a basin with a real first-run choice (seed sweep)", () => {
  ...
  console.log(
    `[si7.5] entry sweep: min flood ${minFlood}/${MAP_WIDTH * MAP_HEIGHT} tiles, min within-capacity POIs ${minWithin} (worst: ${worst})`,
  );
