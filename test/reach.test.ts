...
import { MAP_WIDTH, MAP_HEIGHT, TERRAIN_COST } from "../src/data/constants";
import type { Terrain } from "../src/data/constants";

// costToReach expects a full MAP_WIDTH×MAP_HEIGHT grid, so build a plains
// field and drop features into it for tiny, readable cases.
function plainsGrid(): Terrain[][] {
  return Array.from({ length: MAP_HEIGHT }, () => Array<Terrain>(MAP_WIDTH).fill("plains"));
}
...
test("costToReach: a full mountain wall makes the far side Infinity on foot", () => {
  const g = plainsGrid();
  for (let y = 0; y < MAP_HEIGHT; y++) g[y]![10] = "mountain"; // vertical wall at x=10
...
test("costToReach: climbing-pick opens the wall (far side finite)", () => {
  const g = plainsGrid();
  for (let y = 0; y < MAP_HEIGHT; y++) g[y]![10] = "mountain";
